...
onSubmit(): void {
  if (this.form.valid) {
    ...
    if (!this.originalSeller?.id) {
      this.error = 'Product seller information missing';
      this.loading = false;
      return;
    }
    ...
    Object.keys(this.form.value).forEach(key => {
      if (key !== 'productImageURL' && this.form.value[key] !== null) {
    ...
    // Append seller ID
    formData.append('productSeller', this.originalSeller.id.toString());
