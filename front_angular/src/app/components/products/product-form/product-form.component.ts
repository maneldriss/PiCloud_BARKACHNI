...
onSubmit(): void {
  ...
  // Create product data with only user ID
  const productData = {
    ...this.productForm.value,
    productSeller: { id: currentUser.id } // Send only user ID
  };

  // Remove the user object from the form data
      ...
      .subscribe({
        next: (response) => {
          const product: Product = {
            ...productData,
            productImageURL: `${this.apiUrl}/uploads/` + response.fileName 
          };
