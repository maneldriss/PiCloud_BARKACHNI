...
export class OutfitListComponent implements OnInit, OnDestroy {
  ...
  private recommendationTimeout: any;
  ...
  ngOnInit(): void {
    ...
    if (this.currentUser?.id) {
      this.loadOutfitsAndRecommendation(this.currentUser.id);
      this.loadItemsByCategory(this.currentUser.id);
    } else {
      console.error('No user is logged in');
      this.snackBar.open('Please log in to view your outfits', 'Close', { duration: 3000 });
      this.loading = false;
  ...
  loadOutfitsAndRecommendation(userId: number): void {
    ...
    this.outfitService.getOutfitsByUser(userId).subscribe({
      ...
      error: (error) => {
        ...
        this.snackBar.open(`Failed to load outfits: ${error.message || 'Unknown error'}`, 'Close', { duration: 3000 });
        this.loading = false;
        this.loadingRecommendation = false;
      }
    });
  ...
  applyFilters() {
    ...
    if (this.searchTerm) {
      ...
      result = result.filter(outfit =>
        outfit.outfitName.toLowerCase().includes(term) ||
  ...
  handleImageError(event: Event): void {
    ...
    if (parent) {
      ...
      parent.innerHTML = '<i class="fas fa-image text-muted" style="font-size: 1.5rem;"></i>';
    }
  }
}
