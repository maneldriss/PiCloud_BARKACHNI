  ...
  getOutfitById(id: number) {
    ...
    addOutfit(outfit: Outfit) {
      ...
      return this.userService.getCurrentUser().pipe(
        switchMap(user => {
          if (!user) {
            return throwError(() => new Error("User info missing"));
          }

          console.log('Auth Token Present:', !!this.jwtService.getToken());

          return this.http.post<Outfit>(`${this.apiUrl}/add-outfit`, outfit, { headers })
          .pipe(
            switchMap(createdOutfit => {
              if (user.dressing&&user.dressing.id){
                return this.http.put<Outfit>(
                  `${this.apiUrl}/assign-to-dressing/${createdOutfit.outfitID}/${user.dressing.id}`,
                  {},
                  { headers }
                );
              }
return of(createdOutfit);
              })
            );
        }),
